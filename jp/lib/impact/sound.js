...
ig.SoundManager = ig.Class.extend({
	...
	init: function() {
		// Prefer MP3 over OGG-Vorbis - Chrome currently (7.0) is the only
		// browser supporting both formats, but has (more) problems with OGG
...
ig.Music = ig.Class.extend({
	...
	next: function() {
		...
		this.currentIndex = this.random
			? (Math.random() * this.tracks.length).floor()
